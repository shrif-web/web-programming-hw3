...
app.post(
    ...
    body("content")
        .not()
        .isEmpty()
        .withMessage("filed `content` is not valid."),
    function (req, res) {
...
// DELETE Delete 'C
app.delete("/api/admin/post/crud/:id/", authenticateToken, function (req, res) {
    if (!req.params["id"].match("\\d+")) {
        res.status(400).json({ message: "url id is not valid" });
        return;
    }
...
app.get("/api/admin/post/crud", authenticateToken, function (req, res) {
    let reqEmail = req.user.email;
    MongoClient.connect(databaseURI, function (err, db) {
        db.collection("Users", function (err, users) {
            users.findOne({ email: reqEmail }, function (err, result) {
...
// GET READ with id `CT
app.get("/api/admin/post/crud/:id/", authenticateToken, function (req, res) {
    if (!req.params["id"].match("\\d+")) {
        res.status(400).json({ message: "url id is not valid" });
        return;
    }
...
app.post(
    ...
    body("email")
        .isEmail()
        .normalizeEmail()
        .withMessage("filed `email` is not valid"),
    body("password")
        .isLength({ min: 5 })
        .withMessage("filed `password`.length should be gt 5"),
    function (req, res) {
        // Validation
        if (Object.keys(req.body).length != 2) {
            res.status(400).json({ message: "Request Length should be 2" });
        ...
        MongoClient.connect(databaseURI, function (err, db) {
            db.collection("Users", function (err, users) {
                users.find({ email: emailSent }).toArray(function (err, items) {
                    if (
                        items.length == 0 ||
                        items[0]["password"] != sha256(passwordSent)
                    ) {
                        res.status(401).json({
